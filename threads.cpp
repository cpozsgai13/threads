...
void initialize() {
    //  Call me once, it's great
    //  Call me twice, not so great
    cout << "Initialized" << endl;
}

void readFile(const string& filePath, int sleepMilli) {
    int fileSize = 0;
    call_once(once_, initialize);
...
auto readFileLambda = [](const string& filePath, int sleepMilli) {
    int fileSize = 0;
    call_once(once_, initialize);
...
public:
    ...
    void start() {
        ...
        try {
            ...
            cout << "Thread pool (size " << N << ") started" << endl;
        }
        catch(...) {
            running_ = false;
            throw;  //up
